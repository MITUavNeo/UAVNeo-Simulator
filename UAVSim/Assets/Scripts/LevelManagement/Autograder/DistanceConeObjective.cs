...
/// <summary>
/// A distance cone which completes an autograder task when the drone parks the correct distance away.
/// </summary>
public class DistanceConeObjective : DistanceCone
{
    #region Set In Unity Editor
    /// <summary>
    /// The distance which the drone should park from the cone in cm. 
    /// </summary>
    [SerializeField]
    private float goalDistance = 30;

    /// <summary>
    /// The maximum amount the drone can miss the goal distance in cm and still complete the objective.
    /// </summary>
    [SerializeField]
    ...
    protected override void Update()
    {
        ...
        if (Mathf.Abs(this.Distance - this.goalDistance) < this.allowableDistanceError)
        {
            ...
            if (LevelManager.GetDrone().Physics.LinearVelocity.magnitude < Constants.MaxStopSeed)
            {
