...
public class DronePrefabBuilder : MonoBehaviour
{
    ...
    public static void CreateDronePrefab()
    {
        ...
        // Create root "Player" object (matches old structure where LevelManager does GetComponentInChildren<Racecar>)
        ...
        rb.useGravity = true;
        rb.drag = 0f;             // Flight.cs applies its own aerodynamic drag model
        rb.angularDrag = 0.05f;   // Small value; attitude controller handles rotational damping
        ...
        Racecar racecar = drone.AddComponent<Racecar>();
        ...
        // --- Wire the player cameras into the Racecar component's serialized field ---
        SerializedObject so = new SerializedObject(racecar);
    ...
    /// <summary>
    /// Swaps the car player prefab for the drone in the current scene's LevelManager(s).
