...
/// <summary>
/// Simulates the color and depth channels of the forward-facing RealSense D435 (RGBD)
/// and the downward-facing Arducam 16MP (RGB) cameras.
/// </summary>
public class CameraModule : DroneModule
{
    #region Constants
    /// <summary>
    ...
    /// <summary>
    /// The field of view (in degrees) of the forward RGBD camera.
    /// Based on the Intel RealSense D435i datasheet.
    /// </summary>
    private static readonly Vector2 fieldOfView = new Vector2(69.4f, 42.5f);

    /// <summary>
    /// The field of view (in degrees) of the downward RGB camera.
    /// Based on the Arducam 16MP module.
    /// </summary>
    private const float downwardFoV = 65f;
    ...
    /// <summary>
    /// The GPU-side texture to which the forward color camera renders.
    /// </summary>
    ...
    /// <summary>
    /// The GPU-side texture to which the downward RGB camera renders.
    /// </summary>
    public RenderTexture DownwardRGBImage
    {
        get
        {
            return this.downwardCamera != null ? this.downwardCamera.targetTexture : null;
        }
    }

    /// <summary>
    /// The raw bytes of the color image captured by the forward color camera this frame.
    ...
    /// <summary>
    /// The raw bytes of the image captured by the downward RGB camera this frame.
    /// Each pixel is stored in the ARGB 32-bit format, from top left to bottom right.
    /// </summary>
    public byte[] DownwardRGBImageRaw
    {
        get
        {
            if (!isDownwardImageRawValid)
            {
                this.UpdateDownwardImageRaw();
            }
            return this.downwardImageRaw;
        }
    }
    ...
    /// <summary>
    /// Asynchronously updates and returns the downward RGB image.
    /// Warning: This method blocks for asyncWaitTime ms to wait for the new image to load.
    /// </summary>
    /// <returns>The downward RGB image captured by the camera.</returns>
    public byte[] GetDownwardRGBImageRawAsync()
    {
        this.mustUpdateDownwardImageRaw = true;
        Thread.Sleep(CameraModule.asyncWaitTime);
        return this.downwardImageRaw;
    }
    #endregion
    ...
    /// <summary>
    /// Private member for the DownwardRGBImageRaw accessor.
    /// </summary>
    private byte[] downwardImageRaw;

    /// <summary>
    /// True if downwardImageRaw is up to date for the current frame.
    /// </summary>
    private bool isDownwardImageRawValid = false;

    /// <summary>
    /// The forward-facing color camera (Intel RealSense D435 color channel).
    /// </summary>
    private Camera colorCamera;

    /// <summary>
    /// The forward-facing depth camera (Intel RealSense D435 depth channel).
    /// </summary>
    private Camera depthCamera;

    /// <summary>
    /// The downward-facing RGB camera (Arducam 16MP).
    /// </summary>
    private Camera downwardCamera;
    ...
    /// <summary>
    /// If true, downwardImageRaw is updated next frame.
    /// </summary>
    private bool mustUpdateDownwardImageRaw;
    ...
    protected override void Awake()
    {
        ...
        // The downward camera is the 3rd camera if present
        if (cameras.Length > 2)
        {
            this.downwardCamera = cameras[2];
        }
        ...
        this.downwardImageRaw = new byte[sizeof(float) * CameraModule.ColorWidth * CameraModule.ColorHeight];
    ...
    private void Start()
    {
        ...
        // Create a render texture for the downward camera at runtime
        if (this.downwardCamera != null)
        {
            this.downwardCamera.fieldOfView = CameraModule.downwardFoV;

            RenderTexture downRT = new RenderTexture(CameraModule.ColorWidth, CameraModule.ColorHeight, 24);
            downRT.name = "DownwardRGBImage";
            this.downwardCamera.targetTexture = downRT;

            // Enable the camera so it renders to its RenderTexture each frame.
            // Since it has a target texture, it won't display to the screen directly.
            this.downwardCamera.enabled = true;
    ...
    private void Update()
    {
        ...
        if (this.mustUpdateDownwardImageRaw)
        {
            this.UpdateDownwardImageRaw();
            this.mustUpdateDownwardImageRaw = false;
        }

        if (this.drone.Hud != null)
        {
            this.VisualizeDepth(this.drone.Hud.DepthVisualization);

            // Update the downward RGB feed on the HUD
            if (this.downwardCamera != null && this.downwardCamera.targetTexture != null)
            {
                this.drone.Hud.UpdateDownwardFeed(this.downwardCamera.targetTexture);
            }
        }
    }
    ...
    private void LateUpdate()
    {
        ...
        this.isDownwardImageRawValid = false;
    }
    ...
    /// <summary>
    /// Update downwardImageRaw by rendering the downward camera on the GPU and copying to the CPU.
    /// Warning: this operation is very expensive.
    /// </summary>
    private void UpdateDownwardImageRaw()
    {
        if (this.downwardCamera == null || this.downwardCamera.targetTexture == null)
        {
            return;
        ...
        RenderTexture rt = this.downwardCamera.targetTexture;
        RenderTexture.active = rt;
        this.downwardCamera.Render();
        ...
        for (int r = 0; r < CameraModule.ColorHeight; r++)
        {
            Buffer.BlockCopy(bytes, (CameraModule.ColorHeight - r - 1) * bytesPerRow, this.downwardImageRaw, r * bytesPerRow, bytesPerRow);
        }

        Destroy(image);
        this.isDownwardImageRawValid = true;
    }
    ...
    private void UpdateDepthImage()
    {
        ...
        {
            // Drone prefab is missing the depth camera; warn once and skip rather than NRE every frame.
            // Caller (Update) will still call UpdateDownwardFeed afterward if downwardCamera is wired up.
        ...
        // Settings.DepthDivideFactor can change at runtime (via the settings UI), making
        // the cached depthImage array dimensions stale relative to the current
        // DepthHeight/DepthWidth. Re-allocate if the shape no longer matches.
        if (this.depthImage == null || this.depthImage.Length != height)
        {
            this.depthImage = new float[height][];
            for (int r = 0; r < height; r++)
                this.depthImage[r] = new float[width];
        }

        for (int r = 0; r < height; r++)
        {
            if (this.depthImage[r] == null || this.depthImage[r].Length != width)
                this.depthImage[r] = new float[width];
