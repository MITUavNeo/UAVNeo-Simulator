...
/// <summary>
/// Automatically builds the drone racing course when the FlightDemo scene loads at runtime.
/// This removes the need to manually run the editor menu tool — the course is always present.
///
...
/// </summary>
public static class DroneCourseRuntime
{
    /// <summary>
    /// Register the scene-loaded callback before any scene loads.
    /// </summary>
    ...
    /// <summary>
    /// Fires every time a scene finishes loading. If it's the FlightDemo scene, build the course.
    /// </summary>
    private static void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if (scene.name == "FlightDemo")
        {
            // Always rebuild so code changes (AR patterns, layout, etc.) take effect.
            // BuildCourse() already destroys any existing "DroneCourse" before creating a new one.
            Debug.Log("[DroneCourseRuntime] FlightDemo scene detected — building drone racing course...");
            BuildCourse();
        }
    }

    // ════════════════════════════════════════════════════════════
    //  SHARED MATERIAL CACHE (avoids duplicate allocations)
    // ════════════════════════════════════════════════════════════
    private static Material nYellow;
    private static Material matPillar;   // reused by landing pads
    private static Material matFixture;  // reused by flood lights
    private static Material matPost;     // reused by light posts
    private static Material matARWhite;  // AR marker background
    private static Material matARBlack;  // AR marker foreground
    ...
    public static void BuildCourse()
    {
        ...
        // ═══════════════════════════════════════════════════
        //  MATERIALS (created once, shared everywhere)
        // ═══════════════════════════════════════════════════
        ...
        // Grid lines every 40m — CLIPPED around the line-following area
        // Line-following bounding box (generous margin): X=[25,95], Z=[-110,-12]
        float lfXMin = 25f, lfXMax = 95f, lfZMin = -110f, lfZMax = -12f;
        float gridExtent = 120f; // grid spans ±120 m from origin

        for (int i = -3; i <= 3; i++)
        {
            float coord = i * 40f;

            // Z-running lines (vertical strips at specific X, spanning in Z)
            if (coord > lfXMin && coord < lfXMax)
            {
                // This X falls inside the LF zone — split the line
            ...
            // X-running lines (horizontal strips at specific Z, spanning in X)
        ...
        // Center circle (12 segments instead of 24)
        ...
        // Launch pad markings (each layer clearly above the previous)
        ...
        // Corner pylons (cubes instead of sphere beacons)
        ...
        // Mid altitude climb — AR markers on first gate
        ...
        // High altitude dive — AR markers on first gate
        ...
        // Speed pylons — reduced from 8 to 4
        for (int i = 0; i < 4; i++)
        {
            float z = -30 - i * 20;
            float x = (i % 2 == 0) ? -5 : 5;
        ...
        // ═══════════════════════════════════════════════════
        //  6. TUNNEL (reduced from 6 to 4 segments)
        // ═══════════════════════════════════════════════════
        ...
        // ═══════════════════════════════════════════════════
        //  7. AERIAL SLALOM (reduced from 10 to 6)
        // ═══════════════════════════════════════════════════
        ...
        // ═══════════════════════════════════════════════════
        // 10. FIGURE-8 LOOP (reduced from 16 to 10 gates)
        // ═══════════════════════════════════════════════════
        ...
        for (int i = 0; i < ringCount; i++)
        {
            ...
            Quaternion rot2 = dir.sqrMagnitude > 0.001f ? Quaternion.LookRotation(dir) : Q0;
            Material ringMat = (i % 2 == 0) ? nCyan : nMagenta;
            BuildGate(fig8Parent.transform, $"Fig8_{i}", pos, rot2, 6f, 6f, 0.3f, ringMat, matConcreteD);
        }
        ...
        // ═══════════════════════════════════════════════════
        // 12. DECORATIONS (reduced: 10 crates, no floating orbs)
        // ═══════════════════════════════════════════════════
        ...
        Debug.Log("[DroneCourseRuntime] Drone racing arena built successfully (optimized).");
    }
    ...
    /// <summary>
    /// Creates invisible box-collider walls on all four sides and a ceiling so the
    /// drone cannot fly outside the arena. No visible geometry — purely physics.
    /// </summary>
    static void BuildInvisibleBarriers(Transform parent, float halfSize, float height)
    {
        float thickness = 2f;            // wall thickness
        float fullLen = halfSize * 2f + thickness * 2f; // span including corners
    ...
    static void CreateBarrier(Transform parent, string name, Vector3 pos, Vector3 size)
    {
        ...
        col.size = size;
        go.layer = 0; // Default layer — collides with everything
    }
        ...
        Material accentNeon, Material warningNeon)
    {
        ...
        // Metal posts every 20m (halved from 10m)
        ...
        // Neon accent strips (top and base)
        ...
        int arMarkerId = -1)
    {
        ...
        if (arMarkerId >= 0 && arMarkerId < arMarkerPatterns.Length)
        {
            ...
            float mSize = 0.70f;   // total marker size (7×7 cells)
            float mOffset = 0.04f; // Z offset from gate centre

            // Corner positions (XY only; Z is applied per-face below)
            Vector3[] cornerXY = {
                V(-hw + t/2 + mSize/2, hh - t/2 - mSize/2, 0),   // top-left
                V( hw - t/2 - mSize/2, hh - t/2 - mSize/2, 0),   // top-right
                V(-hw + t/2 + mSize/2, -hh + t/2 + mSize/2, 0),  // bottom-left
                V( hw - t/2 - mSize/2, -hh + t/2 + mSize/2, 0),  // bottom-right
            };

            // Place on both sides (+Z and -Z) so markers are visible from any direction.
    ...
    /// 1 = white cell, 0 = black cell.  The marker has a 1-cell black border
    /// around the 5×5 data, giving a 7×7 visual grid — standard ArUco style.
    /// Detectable by any OpenCV ArUco detector using DICT_5X5_50.
    ...
    /// <summary>
    /// Builds one 5×5 ArUco-style marker at the given position.
    /// Structure: white background → black body (7×7) → white cells for each "1" in the pattern.
    ...
    static void Build5x5Marker(Transform parent, string name, Vector3 pos,
        ...
        float zSign = 1f)
    {
        float cell = totalSize / 7f;       // 7×7 grid: 1-cell border + 5×5 data
        float depth = 0.015f;              // thin slab

        // Layer 0: White background (slightly larger for contrast)
        Box($"{name}_bg", parent, pos, Q0, V(totalSize + 0.04f, totalSize + 0.04f, depth), whiteMat);

        // Layer 1: Black body covering the full 7×7 area (the border)
        Box($"{name}_body", parent, pos + V(0, 0, zSign * depth * 0.5f), Q0,
            V(totalSize, totalSize, depth), blackMat);

        // Layer 2: White cells for each "1" in the 5×5 pattern
        float cellZ = zSign * depth * 1.1f; // sits just in front of the black body
        ...
        float length, float width, float height, int segments, Material wallMat, Material lightMat)
    {
        ...
        for (int i = 0; i < segments; i++)
        {
            ...
            // LED strip on ceiling (every other segment)
            if (i % 2 == 0)
        ...
        float length, float width, float height, Material wallMat, Material lightMat)
    {
        ...
        int segs = Mathf.CeilToInt(length / 15f); // larger segments = fewer objects
    ...
    // ════════════════════════════════════════════════════════════
    //  LIGHTING SETUP (optimized: shared materials, simpler)
    // ════════════════════════════════════════════════════════════
    static void SetupLighting(Transform root)
    {
        ...
        foreach (Light l in lights)
        {
            if (l.type == LightType.Directional)
            {
                ...
                // Let PerformanceOptimizer decide shadow quality based on hardware
                l.shadows = LightShadows.Hard; // Hard shadows (cheapest visible option)
                l.shadowBias = 0.05f;
        ...
        RenderSettings.ambientLight = new Color(0.08f, 0.09f, 0.14f); // Slightly brighter ambient to compensate less lights

        // Linear fog is cheaper than ExponentialSquared on old GPUs
        ...
        // Stadium flood lights (simplified, shared materials)
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            flood.range = 200f;  // Reduced from 250 — perf saving on draw calls
            flood.spotAngle = 90f;
            flood.shadows = LightShadows.None;
            flood.renderMode = LightRenderMode.Auto; // Let Unity decide pixel vs vertex
    ...
    //  Segments go point-to-point so there are no gaps.
    // ════════════════════════════════════════════════════════════

    static void BuildLineFollowingCourse(Transform parent, Material floorMat)
    {
        // ── UNLIT materials: completely bypass the lighting pipeline ──
        // Standard-shader lighting calculations (specular, shadow, ambient)
        // amplify tiny depth differences into visible per-frame flicker.
        // Unlit/Color renders a flat colour with ZERO lighting = ZERO flicker.
        ...
        float tapeW = 0.15f;  // tape width  (≈15 cm)
        float tapeH = 0.008f; // tape height
        float padY  = 0.10f;  // pad centre Y  — 10 cm above floor (9 cm gap)
        float y     = 0.20f;  // tape centre Y — 20 cm above floor (well above pad top)

        // ─────────────────────────────────────────────────
        //  COURSE A — Blue oval loop (closed)
        //  centre ≈ (50, 0, -35),  straight 20m + semicircle R=8
        // ─────────────────────────────────────────────────
        {
            ...
            pts.Add(V(c.x + halfL, y, c.z + R));
            // Right semicircle (top to bottom, 12 segs)
            ArcPoints(pts, V(c.x + halfL, y, c.z), R, 90f, -90f, 12);
            // Bottom straight (right to left)
            pts.Add(V(c.x - halfL, y, c.z - R));
            // Left semicircle (bottom to top, 12 segs)
            ArcPoints(pts, V(c.x - halfL, y, c.z), R, -90f, 90f, 12);
            // Close the loop
            pts.Add(pts[0]);
        ...
        // ─────────────────────────────────────────────────
        //  COURSE B — Green figure-8 (two separate loops to avoid
        //  self-crossing Z-fight at the centre)
        // ─────────────────────────────────────────────────
        {
            ...
            // Left circle (slightly higher to avoid overlap at crossing)
            float y2 = y + 0.02f;
        ...
        // ─────────────────────────────────────────────────
        //  COURSE C — Red zigzag (connected end-to-end)
        // ─────────────────────────────────────────────────
        {
            Vector3 c = V(80f, 0f, -35f);
            UnlitBox("Pad_C", parent, V(c.x, padY, c.z), Q0, V(14, 0.02f, 30), padMat);

            float zigAmp = 4f, zigStep = 4f;
            int zigCount = 8;
            float startZ = c.z - (zigCount * zigStep) / 2f;

            var pts = new System.Collections.Generic.List<Vector3>();
            for (int i = 0; i <= zigCount; i++)
            {
                float zz = startZ + i * zigStep;
        ...
        //  Straight → 90° right → straight → 90° right → chicane → 90° right → straight → 90° right
        // ─────────────────────────────────────────────────
        {
        ...
        //  Two semicircles facing opposite directions
        // ─────────────────────────────────────────────────
        {
            ...
            // Right semicircle (curves right)
            ArcPoints(pts, V(c.x + R, y, c.z - R), R, 180f, 0f, 10);
            // Left semicircle (curves left) — centre shifted so arcs connect
        ...
        //  Tests colour priority / decision-making
        // ─────────────────────────────────────────────────
        {
            ...
            float L = 10f;
            // Each crossing line gets its own render queue so they NEVER Z-fight (Unlit)
    ...
    /// <summary>
    /// Draws connected tape segments between consecutive waypoints.
    /// Each segment is inset slightly from both endpoints so adjacent
    /// segments never overlap at junctions — eliminates Z-fighting.
    /// </summary>
    ...
    {
        const float inset = 0.02f; // pull each end back by this much (invisible from above)
        ...
        for (int i = 0; i < waypoints.Count - 1; i++)
        {
            ...
            // Disable shadow interaction — flat ground decals don't need shadows
    ...
    /// <summary>
    /// Appends arc waypoints to an existing list so arcs connect seamlessly
    /// to whatever came before.  startDeg / endDeg in Unity world coordinates
    /// (0° = +X, 90° = +Z).
    /// </summary>
    ...
    {
        // Start from segment 1 (segment 0 would duplicate the previous endpoint)
    ...
    static void CleanExistingGround()
    {
        ...
        foreach (GameObject rootObj in rootObjects)
        {
            foreach (string gn in groundNames)
            {
                if (rootObj.name == gn)
                {
                    Debug.Log($"[DroneCourseRuntime] Removing scene ground: {rootObj.name}");
    ...
    static Material CreateMat(string name, Color color, int renderQueueOffset = 0)
    {
        ...
        {
            mat.renderQueue = 2000 + renderQueueOffset; // Geometry queue = 2000
    ...
    /// <summary>
    /// Creates an Unlit/Color material that completely bypasses the lighting pipeline.
    /// No specular, no shadows, no normal-based shading = zero lighting-related flicker.
    /// </summary>
    ...
    /// <summary>
    /// Creates a Box primitive with shadow casting/receiving disabled.
    /// Used for flat ground decals (pads, tape) that must never interact with lighting.
    /// </summary>
