...
public class MainMenu : MonoBehaviour
{
    ...
    /// <summary>
    /// The game object containing the elements which allow the user to select the number of drones to spawn in the level.
    /// </summary>
    [SerializeField]
    private GameObject numDrones;
    #endregion
    ...
    public void BeginSimulation()
    {
        ...
        LevelManager.NumPlayers = this.dropdowns[(int)Dropdowns.NumDrones].value + 1;
    ...
    /// <param name="numDrones">The number of drones which should be selected.</param>
    public void HandleLevelCollectionDropdownChange(int selectedLevel, LevelManagerMode mode, int numDrones)
    {
        ...
        this.HandleLevelDropdownChange(mode, numDrones);
    }
    ...
    /// <param name="mode">The LevelManagerMode which should be selected.</param>
    /// <param name="numDrones">The number of drones which should be selected.</param>
    public void HandleLevelDropdownChange(LevelManagerMode mode, int numDrones)
    {
        ...
        // Show and populate the numDrones dropdown if the level supports multiple drones
        Dropdown numDroneDropdown = this.dropdowns[(int)Dropdowns.NumDrones];
        if (this.SelectedLevel.MaxDrones > 1)
        {
            if (this.SelectedLevel.MaxDrones != numDroneDropdown.options.Count)
            {
                List<string> options = new List<string>(this.SelectedLevel.MaxDrones);
                for (int i = 1; i <= this.SelectedLevel.MaxDrones; i++)
                {
                    options.Add(i.ToString());
                }
                numDroneDropdown.ClearOptions();
                numDroneDropdown.AddOptions(options);
            }
            this.numDrones.SetActive(true);
        }
        else
        {
            this.numDrones.SetActive(false);
        }
        ...
        numDroneDropdown.value = numDrones - 1;
    }

    /// <summary>
    /// Handles when the user selects a new value in the num drones dropdown.
    /// </summary>
    public void HandleNumDronesChange()
    {
        // Lock the mode dropdown to "Race" if the user chcose multiple drones
        Dropdown modeDropdown = this.dropdowns[(int)Dropdowns.Mode];
        if (this.dropdowns[(int)Dropdowns.NumDrones].value > 0)
        {
